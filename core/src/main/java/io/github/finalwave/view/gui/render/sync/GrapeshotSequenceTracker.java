...
public final class GrapeshotSequenceTracker {
    ...
    public void onSpawn(Plant plant) {
        if (plant == null || !plant.isGrapeshot()) {
            return;
        }
        pending.add(plant);
    }
    ...
    public boolean startSequenceIfNeeded(Plant plant, PamActor actor, PlantClips clips, float scale) {
        if (plant == null || actor == null || !plant.isGrapeshot()) {
            return false;
        }
        ...
        started.add(plant);
        var idle = clips.grapeshotIdle();
        var attack = clips.grapeshotAttack(plant);
