...
public final class JalapenoSequenceTracker {
    ...
    public void onSpawn(Plant plant) {
        if (plant == null || !plant.isJalapeno()) {
            return;
        }
        pending.add(plant);
    }
    ...
    public boolean startSequenceIfNeeded(Plant plant, PamActor actor, PlantClips clips, float scale) {
        if (plant == null || actor == null || !plant.isJalapeno()) {
            return false;
        }
        ...
        started.add(plant);
        var idle = clips.jalapenoIdle();
        var attack = clips.jalapenoAttack();
