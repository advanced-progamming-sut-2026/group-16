...
public final class PhatBeetPulseSync {
    private final GameAssets assets;
    private final LawnLayout layout;
    private final Group layer;

    public PhatBeetPulseSync(GameAssets assets, LawnLayout layout, Group layer) {
        this.assets = assets;
        this.layout = layout;
        this.layer = layer;
    }
    ...
    public void sync(GameSession session) {
        if (session == null || session.getPhatBeetPulseSystem() == null) {
            return;
        }
        List<PhatBeetPulseMark> marks = session.getPhatBeetPulseSystem().drainPulseMarks();
        for (PhatBeetPulseMark mark : marks) {
            if (mark == null) {
                continue;
            }
            var pulse = mark.plantFood() ? PhatBeetClips.plantFoodPulse() : PhatBeetClips.attackPulse();
            spawnFx(mark.plantCol(), mark.plantRow(), pulse);
            var hit = PhatBeetClips.tileHit();
            for (PhatBeetPulseMark.HitTile tile : mark.hits()) {
                if (tile != null) {
